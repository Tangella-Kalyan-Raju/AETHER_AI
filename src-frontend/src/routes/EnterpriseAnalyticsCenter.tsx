...
const KPICard = ({ kpi, icon: Icon }: { kpi: KPI; icon: any }) => {
  const isPositiveTrend =
    (kpi.trend_direction === "UP" && kpi.percentage_change > 0) ||
    (kpi.trend_direction === "DOWN" && kpi.name === "Grid Loss");
...
export default function EnterpriseAnalyticsCenter() {
  const [activeTab, setActiveTab] = useState<"executive" | "trends" | "comparison" | "reports">(
    "executive"
  );
            ...
            <div className="space-y-6">
              <div>
                <h4 className="text-sm font-bold text-slate-500 uppercase tracking-wider mb-2 flex items-center gap-2">
                  <Activity className="w-4 h-4" /> Operational Performance
                </h4>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                  <h4 className="text-sm font-bold text-green-500 uppercase tracking-wider mb-2 flex items-center gap-2">
    ...
    return (
      <div className="space-y-6 animate-in slide-in-from-bottom-4 duration-500">
        ...
        <p className="text-slate-500">
          Visualizing the direct impact of the applied policy optimizations.
        </p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {comparisons.metrics.map((metric: any, idx: number) => {
            ...
            const diffColor =
              (metric.name === "Renewable Mix" && diff > 0) ||
              (metric.name !== "Renewable Mix" && diff < 0)
                ? "text-emerald-500"
                : "text-orange-500";
    ...
    return (
      <Card className="p-12 text-center border-dashed bg-slate-50/50 dark:bg-[#0B0E13]/50 animate-in fade-in duration-500">
        ...
        <p className="text-slate-500 dark:text-slate-400 max-w-md mx-auto mb-6">
          The analytics schemas are prepared for PDF and Excel generation. (Export functionality
          excluded as per Phase 5.5 constraints).
        </p>
