...
export default function SimulationControlCenter() {
  ...
  const fetchScenarios = async () => {
    try {
      ...
      console.warn("API Scenarios issue");
    }
  };
  ...
  const fetchHistory = async () => {
    try {
      ...
      console.warn("API History issue");
    }
  };
            ...
            <div className="p-6 rounded-xl bg-white dark:bg-[#0B0E13] border border-slate-200 dark:border-[#1E293B] shadow-sm">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {scenarios.map((s) => (
                  <div
                    key={s.id}
                    className="p-4 rounded-lg bg-slate-50 dark:bg-[#151A21] border border-slate-200 dark:border-[#2A313C] hover:border-orange-500/50 transition-colors flex flex-col justify-between"
                  >
                    <div>
                      <div className="flex justify-between items-start mb-2">
                        <h3 className="font-bold text-slate-900 dark:text-white">{s.name}</h3>
                        <span className="text-[10px] uppercase font-bold text-slate-500 bg-slate-200 dark:bg-[#1E293B] px-1.5 py-0.5 rounded">
                          {s.scenario_type}
                        </span>
                      </div>
                    ...
                    <div className="flex gap-2">
                      <button
                        onClick={() => startSimulation(s.id, 1.0)}
                        className="flex-1 py-1.5 bg-emerald-600 hover:bg-emerald-500 text-white text-xs font-bold rounded flex items-center justify-center gap-1"
                      >
                        <Play className="w-3 h-3" /> Run
                      </button>
                      <button
                        onClick={() => startSimulation(s.id, 10.0)}
                        className="flex-1 py-1.5 bg-amber-600 hover:bg-amber-500 text-white text-xs font-bold rounded flex items-center justify-center gap-1"
                      >
            ...
            <div className="p-6 rounded-xl bg-white dark:bg-[#0B0E13] border border-slate-200 dark:border-[#1E293B] shadow-sm min-h-[300px]">
              ...
              <div className="space-y-3">
                {history.map((h) => (
                  <div
                    key={h.id}
                    className="p-3 rounded bg-slate-50 dark:bg-[#151A21] border border-slate-200 dark:border-[#2A313C] flex justify-between items-center cursor-pointer hover:border-blue-500/50"
                  >
                    <div>
                      <p className="text-sm font-bold text-slate-900 dark:text-white">
                        Sim {h.id.substring(0, 6)}
                      </p>
                    ...
                    <span
                      className={`text-[10px] uppercase font-bold px-2 py-1 rounded ${h.status === "COMPLETED" ? "bg-emerald-500/10 text-emerald-500" : "bg-red-500/10 text-red-500"}`}
                    >
                      {h.status}
                    </span>
                  </div>
              ...
              <div>
                ...
                <div className="flex gap-4 mt-1">
                  <span className="text-xs font-mono text-slate-500">
                    T+{simState?.current_time || 0} mins
                  </span>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
            <div className="lg:col-span-8 flex flex-col gap-6">
              ...
              <div className="p-5 rounded-xl bg-white dark:bg-[#0B0E13] border border-slate-200 dark:border-[#1E293B]">
                <div className="flex justify-between items-center mb-4">
                  <h3 className="font-bold flex items-center gap-2">
                    <Activity className="w-4 h-4 text-emerald-500" /> Execution Timeline
                  </h3>
                </div>
                <div className="relative h-2 bg-slate-200 dark:bg-[#1E293B] rounded-full overflow-hidden">
                  <div
                    ...
                    style={{
                      width: `${Math.min(100, ((simState?.current_time || 0) / 1440) * 100)}%`,
                    }}
                  ></div>
                </div>
              ...
              <div className="p-5 rounded-xl bg-white dark:bg-[#0B0E13] border border-slate-200 dark:border-[#1E293B] flex-1 min-h-[400px] flex flex-col items-center justify-center relative overflow-hidden">
                ...
                <p className="text-xs text-slate-400 max-w-sm text-center mt-2">
                  The digital twin will render geospatial impact markers here in real-time as the
                  simulation timeline progresses.
                </p>
              </div>
            </div>
            ...
            <div className="lg:col-span-4 flex flex-col gap-6">
              <div className="p-5 rounded-xl bg-white dark:bg-[#0B0E13] border border-slate-200 dark:border-[#1E293B] flex-1">
                <h3 className="font-bold flex items-center gap-2 mb-4 text-sm">
                  <ShieldAlert className="w-4 h-4 text-amber-500" /> Live Event Log
                </h3>
                <div className="space-y-3 h-[500px] overflow-auto pr-2">
                  {(simState?.recent_events || []).map((evt: any, i: number) => (
                    <div
                      key={i}
                      className="p-3 bg-slate-50 dark:bg-[#151A21] border border-slate-200 dark:border-[#2A313C] rounded-lg"
                    >
