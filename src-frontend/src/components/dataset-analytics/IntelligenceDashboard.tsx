  ...
  return (
    <div className="space-y-6 mt-6 animate-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-4">
        <KPICard
          title="Total Records"
          value={exec.total_records?.toLocaleString()}
          icon={<Activity />}
        />
        ...
        <KPICard
          ...
          value={`${gen.avg_generation?.toFixed(1)} MW`}
          icon={<Battery />}
        />
        <KPICard
          title="Data Quality"
          value={`${exec.data_quality_score?.toFixed(1)}%`}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-1 rounded-lg border border-slate-800 bg-[#07090C]/40 p-5 space-y-4">
          ...
          <div className="space-y-3 text-sm">
            ...
            <SummaryRow
              label="Time Period"
              value={`${new Date(exec.start_date).toLocaleDateString()} - ${new Date(exec.end_date).toLocaleDateString()}`}
            />
            <SummaryRow label="Total Columns" value={exec.total_columns} />
            <SummaryRow
              label="Missing Data"
              value={`${exec.missing_values_pct?.toFixed(2)}%`}
              color={exec.missing_values_pct > 5 ? "text-red-400" : "text-emerald-400"}
            />
            <SummaryRow
              label="Duplicate Rows"
              value={exec.duplicate_records}
              color={exec.duplicate_records > 0 ? "text-yellow-400" : "text-emerald-400"}
            />
            <SummaryRow
              label="Overall Readiness"
              value={`${exec.overall_health_pct?.toFixed(1)}%`}
              color={exec.overall_health_pct > 90 ? "text-emerald-400" : "text-yellow-400"}
            />
          </div>
        </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="space-y-3">
              ...
              {ai.insights?.map((insight: any, i: number) => (
                <div
                  key={i}
                  className="p-3 bg-slate-900/50 border border-slate-800 rounded text-xs text-slate-300"
                >
            ...
            <div className="space-y-3">
              ...
              {ai.recommendations?.map((rec: any, i: number) => (
                ...
                >
                  <div className="flex justify-between items-start mb-1">
                    <span
                      className={`font-semibold ${rec.priority === "High" ? "text-red-400" : "text-blue-400"}`}
                    >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="rounded-lg border border-slate-800 bg-[#07090C]/40 p-5 space-y-4">
          ...
          <div className="h-64">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart
                data={Object.entries(regional).map(([region, data]: any) => ({
                  name: region,
                  demand: data.demand,
...
function KPICard({ title, value, icon }: { title: string; value: any; icon: React.ReactNode }) {
  return (
    <div className="p-4 rounded-lg border border-slate-800 bg-[#07090C]/60 flex items-center gap-4">
      <div className="p-2 bg-slate-800/50 rounded-md text-slate-400">{icon}</div>
      <div>
