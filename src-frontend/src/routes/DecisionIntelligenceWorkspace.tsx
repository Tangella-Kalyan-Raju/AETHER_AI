...
export default function DecisionIntelligenceWorkspace() {
  const [activeWorkspace, setActiveWorkspace] = useState<
    "policy" | "weather" | "scenario" | "tradeoff"
  >("policy");
  ...
  const radarData = scores
    ? [
        ...
        {
          subject: "Reliability",
          A: scores["Balanced Strategy"]?.reliability || 90,
          fullMark: 100,
        },
        {
          subject: "Sustainability",
          A: scores["Balanced Strategy"]?.sustainability || 85,
          fullMark: 100,
        },
        { subject: "Risk Mitigation", A: scores["Balanced Strategy"]?.risk || 80, fullMark: 100 },
        {
          subject: "Grid Stability",
          A: scores["Balanced Strategy"]?.stability || 92,
          fullMark: 100,
        },
      ]
    : [];
    ...
    <div className="space-y-6 animate-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <Card className="p-6 lg:col-span-1 border-dashed bg-slate-50/50 dark:bg-[#0B0E13]/50">
          <h4 className="text-sm font-bold text-slate-900 dark:text-white mb-4 uppercase tracking-wider text-center">
            Balanced Strategy Score
          </h4>
          <div className="h-64">
          ...
          <div className="text-center mt-4">
            <span className="text-3xl font-bold text-orange-500">
              {scores?.["Balanced Strategy"]?.overall || 86.6}
            </span>
          ...
          <div className="h-72">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart
                data={[
                  {
                    metric: "Operating Cost ($K)",
                    "Max Reliability": 140,
                    "Balanced Strategy": 115,
                    "Cost Focus": 95,
                  },
                  {
                    metric: "CO2 Emissions (T)",
                    "Max Reliability": 850,
                    "Balanced Strategy": 650,
                    "Cost Focus": 900,
                  },
                  {
                    metric: "Grid Loss (MW)",
                    "Max Reliability": 30,
                    "Balanced Strategy": 35,
                    "Cost Focus": 48,
                  },
                ]}
              >
      ...
      <div className="space-y-4">
        {recommendations.map((rec, i) => (
          <Card
            key={i}
            className={`p-6 border-l-4 ${rec.rank === 1 ? "border-l-orange-500 bg-orange-500/5" : "border-l-slate-400 bg-slate-50 dark:bg-[#151A21]"}`}
          >
            <div className="flex justify-between items-start mb-4">
              <div className="flex items-center gap-3">
                <div
                  className={`w-8 h-8 rounded-full flex items-center justify-center font-bold ${rec.rank === 1 ? "bg-orange-500 text-white" : "bg-slate-200 dark:bg-slate-700 text-slate-600 dark:text-slate-300"}`}
                >
                  #{rec.rank}
                </div>
              ...
              <div>
                ...
                <ul className="space-y-1">
                  {rec.advantages.map((adv: string, j: number) => (
                    ...
                    >
                      <CheckCircle2 className="w-3 h-3 text-emerald-500" /> {adv}
                    </li>
                  ))}
                </ul>
              </div>
                ...
                <ul className="space-y-1">
                  {rec.disadvantages.map((dis: string, j: number) => (
                    <li
                      ...
                      className="text-sm text-slate-600 dark:text-slate-400 flex items-center gap-2"
                    >
                      <AlertTriangle className="w-3 h-3 text-orange-500" /> {dis}
                    </li>
                  ))}
                </ul>
              </div>
    ...
    <div className="space-y-6 animate-in slide-in-from-bottom-4 duration-500">
      ...
      <Card className="p-6">
        <p className="text-sm text-slate-500 mb-6">
          Evaluating current operational plan resilience against severe meteorological phenomena.
        </p>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="p-4 rounded border border-blue-500/30 bg-blue-500/5">
            <h4 className="font-bold text-blue-500 flex items-center gap-2 mb-4">
              <CheckCircle2 className="w-4 h-4" /> Normal Conditions
            </h4>
            <div className="space-y-2 text-sm text-slate-600 dark:text-slate-300">
              <div className="flex justify-between">
                <span>Load Variance:</span> <span>0%</span>
              </div>
              <div className="flex justify-between">
                <span>Solar Yield:</span> <span>95%</span>
              </div>
              <div className="flex justify-between">
                <span>Reliability Risk:</span> <Badge variant="secondary">Low</Badge>
              </div>
            </div>
          </div>
          <div className="p-4 rounded border border-orange-500/30 bg-orange-500/5 relative overflow-hidden">
            ...
            <h4 className="font-bold text-orange-500 flex items-center gap-2 mb-4">
              <AlertTriangle className="w-4 h-4" /> Heatwave (+10°C)
            </h4>
            <div className="space-y-2 text-sm text-slate-600 dark:text-slate-300">
              <div className="flex justify-between">
                <span>Load Variance:</span>{" "}
                <span className="text-orange-500 font-bold">+18% (HVAC)</span>
              </div>
              <div className="flex justify-between">
                <span>Solar Yield:</span> <span>88% (Heat derating)</span>
              </div>
              <div className="flex justify-between">
            ...
            <p className="text-xs text-orange-600 mt-4 font-medium border-t border-orange-500/20 pt-2">
              Requires immediate +300MW reserve allocation.
            </p>
          </div>
          <div className="p-4 rounded border border-slate-700 bg-slate-900">
            <h4 className="font-bold text-slate-300 flex items-center gap-2 mb-4">
              <Zap className="w-4 h-4 text-emerald-400" /> Heavy Storm
            </h4>
            <div className="space-y-2 text-sm text-slate-400">
              <div className="flex justify-between">
                <span>Load Variance:</span> <span>-5%</span>
              </div>
              <div className="flex justify-between">
                <span>Wind Yield:</span>{" "}
                <span className="text-emerald-400 font-bold">120% (High gusts)</span>
              </div>
              <div className="flex justify-between">
                <span>Reliability Risk:</span>{" "}
  ...
  const renderTradeOffDashboard = () => {
    ...
    return (
      <div className="space-y-6 animate-in slide-in-from-bottom-4 duration-500">
        ...
        <Card className="p-6">
          <p className="text-sm text-slate-500 mb-6">
            Scatter plot identifying the Pareto frontier between Operational Cost and System
            Reliability.
          </p>
          <div className="h-80">
            <ResponsiveContainer width="100%" height="100%">
    ...
    <div className="p-6 md:p-8 space-y-8 pb-32 max-w-[1600px] mx-auto">
      ...
      {isComparing ? (
        <div className="flex flex-col items-center justify-center py-24 animate-in fade-in duration-500">
          ...
          <h3 className="text-xl font-bold text-slate-900 dark:text-white">
            Synthesizing Enterprise Comparisons...
          </h3>
          <p className="text-slate-500 mt-2">
            Correlating multi-dimensional KPIs across operational models.
          </p>
        </div>
      ) : (
        <>
          ...
          {activeWorkspace === "scenario" && (
            <Card className="p-12 text-center border-dashed flex flex-col items-center justify-center bg-slate-50/50 dark:bg-[#0B0E13]/50">
              ...
              <p className="text-slate-500 dark:text-slate-400 max-w-md mx-auto">
                Historical and simulated scenario comparison tracking is active. Render views for
                multi-scenario tables will arrive in Phase 5.5 Enterprise Reporting.
              </p>
            </Card>
          )}
        </>
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-blue-500/10 border-t border-blue-500/20 p-3 flex justify-center backdrop-blur-md z-50">
        <p className="text-xs text-blue-600 dark:text-blue-400 font-medium flex items-center gap-2">
          <Activity className="w-4 h-4" /> <strong>Advisory Mode:</strong> The Decision Intelligence
          Engine does not automatically execute operational policies. All recommendations must be
          manually accepted by control room operators.
        </p>
      </div>
    </div>
  );
